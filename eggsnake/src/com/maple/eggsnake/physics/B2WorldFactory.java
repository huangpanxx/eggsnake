...
public class B2WorldFactory {
	...
	public static Body loadBody(String path, World world) {
...
//		Loggable logger = DefaultLogger.getDefaultLogger();
		...
		B2Body b = gson.fromJson(reader, B2Body.class);
		return createBody(b, world);
	}
	...
	// 添加Body
	static Body createBody(B2Body b, World world) {
			...
			for (B2Fixture fd : b.fixture) {
				...
				if (fd.circle != null) {
					...
					for (int i = 0; i < size; ++i) {
						vertices[i] = new Vector2(fd.chain.vertices.x.get(i)
								.toFloat(), fd.chain.vertices.y.get(i)
								.toFloat());
					}
					...
					Vector2 vertices[] = new Vector2[size];
					for (int i = 0; i < size; ++i) {
						vertices[i] = new Vector2(fd.polygon.vertices.x.get(i)
								.toFloat(), fd.polygon.vertices.y.get(i)
								.toFloat());
					}
